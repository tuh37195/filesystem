...
superblock* init_super(int superblock_max, int inode_max, int inode_size, int block_size, char *disk_name){
    //allocate memory
    superblock *sb = (superblock*)malloc(sizeof(superblock));
    //set inode location
    sb->inode_locaton = superblock_max+1;
    sb->inode_count = 0;
    sb->inode_max = inode_max;
    //get disk size
    int disk_size = get_disk_size(disk_name);
    //subtract size of superblock and inode locations
    disk_size -= superblock_max + (inode_size * inode_max);
    //get the maximum number of blocks
    int block_count = disk_size/block_size + (disk_size % block_size != 0);
    sb->block_count = block_count;
    //break up the block into 32bit chunks
    long int chunks = block_count/sizeof(long int) + (block_count % sizeof(int));
